...
//                System.out.println("The only algorithm available is First-Fit (ff)");
                ...
                } else if (ALGORITHM_CHOICE_LEAST_TURNAROUND_TIME.equals(args[i + 1])) {
                    clnt = new LeastTurnaroundTimeSchedulerImpl(isVerbose);
                } else {
                    System.out.println("Err: invalid algorithm (" + args[i + 1] + ")");
                    System.out.println("Usage: java -jar client.jar [-h] [-v] [-a algo_name]");
                    System.out.println("The algorithms available are First-Fit (ff) and Least Turnaround Time (lt)");
                    System.exit(0);
                }
            }
        }
